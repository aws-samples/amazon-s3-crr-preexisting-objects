...
aws emr create-cluster \
  ...
  --steps '[{"Args":["s3://'$INVENTORY_BUCKET'/emr/step_0.sh","s3://'$INVENTORY_BUCKET'/emr/copy_objects.py"],"Type":"CUSTOM_JAR","ActionOnFailure":"CONTINUE","Jar":"s3://'$REGION'.elasticmapreduce/libs/script-runner/script-runner.jar","Properties":"","Name":"Download Spark Script"},
            {"Args":["spark-submit","--deploy-mode","client","/home/hadoop/copy_objects.py","'$GLUE_DATABASE_NAME'.'$ATHENA_TABLE_NAME'","'$INVENTORY_DATE'","s3://'$INVENTORY_BUCKET'/results/", "--partitions", "'$PARTITIONS'"],"Type":"CUSTOM_JAR","ActionOnFailure":"CONTINUE","Jar":"command-runner.jar","Properties":"","Name":"Run Spark Application"}]'
